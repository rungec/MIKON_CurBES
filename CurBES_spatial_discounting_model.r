...
#model selection
g1 <- lmer(logdist2road ~ age + income + education + activity*gender + (1|LogID), data = ppgis_sub)
g2 <- lmer(logdist2road ~ age + education + activity*gender + (1|LogID), data = ppgis_sub)
g3 <- lmer(logdist2road ~ income + education + activity*gender + (1|LogID), data = ppgis_sub)
...
g7 <- lmer(logdist2road ~ education + activity + gender + (1|LogID), data = ppgis_sub)
g8 <- lmer(logdist2road ~ age + education + activity + gender + (1|LogID), data = ppgis_sub)
g9 <- lmer(logdist2road ~ age + income + education + activity + gender + (1|LogID), data = ppgis_sub)
g10 <- lmer(logdist2road ~ income + education + activity + (1|LogID), data = ppgis_sub)
g11 <- lmer(logdist2road ~ age + education + activity + (1|LogID), data = ppgis_sub)
g12 <- lmer(logdist2road ~ age + activity + (1|LogID), data = ppgis_sub)
g12b <- lmer(logdist2road ~ education + activity + (1|LogID), data = ppgis_sub)
g12c <- lmer(logdist2road ~ income + activity + (1|LogID), data = ppgis_sub)
...
g15 <- lmer(logdist2road ~ activity + education + (1|LogID), data = ppgis_sub)
...
g12b <- lmer(logdist2road ~ education + activity + (1|LogID), data = ppgis_sub2)
g12c <- lmer(logdist2road ~ income + activity + (1|LogID), data = ppgis_sub2)
...
p<-plot_model(g13, type = "diag")
pout <- ggarrange(p[[1]], p[[2]]$LogID, p[[3]], p[[4]], ncol=2, nrow=2, labels = c("A", "B", "C", "D"))
ggsave("Model_of_dist2road_bysocioecon_lme_dropprimaryeducated_diag_bestmod.png", pout, width=9, height=7, units="in", dpi=300)

#plot the model diagnostics, fullmod
p<-plot_model(g1, type = "diag")
pout <- ggarrange(p[[1]], p[[2]]$LogID, p[[3]], p[[4]], ncol=2, nrow=2, labels = c("A", "B", "C", "D"))
ggsave("Model_of_dist2road_bysocioecon_lme_dropprimaryeducated_diag_fullmod.png", pout, width=9, height=7, units="in", dpi=300)
...
pout <- ggarrange(p5, ggarrange(p1, p2, p3, p4, ncol = 2, nrow=2, labels = c("B", "C", "D", "E")), # Second row with box and dot plots
                  nrow = 2, labels = "A" )   
ggsave("Model_of_dist2road_bysocioecon_lme_dropprimaryeducated_marginaleffects_fullmod.png", pout, width=7, height=9, units="in", dpi=300)
